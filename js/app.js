(() => {
    ...
    let _slideUp = (target, duration = 200, showmore = 0) => {
    ...
    let _slideToggle = (target, duration = 200) => {
    ...
    for (let i = 0; i < langs.length; i++) {
        langs[i];
        const langSelected = document.querySelector(".lang-header__selected");
        const langList = document.querySelector(".lang-header__list");
        const langItem = document.querySelectorAll(".lang-header__list span");
        _slideUp(langList);
        langSelected.addEventListener("click", (function() {
            _slideToggle(langList);
        }));
        langItem.forEach((item => {
            item.addEventListener("click", (function() {
                langSelected.textContent = item.textContent;
                _slideUp(langList);
            }));
        }));
        document.addEventListener("click", (function(e) {
            if (!langList.contains(e.target) && e.target !== langSelected) _slideUp(langList);
        }));
    }
    window["FLS"] = false;
    isWebp();
